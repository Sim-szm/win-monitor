...
void CButtonST::PaintBk(CDC* pDC)
{
	...
	if (m_dcBk.m_hDC == NULL)
	{
		...
		m_bmpBk.CreateCompatibleBitmap(&clDC, rect.Width(), rect.Height());    //开始就创建画布了？？
		m_pbmpOldBk = m_dcBk.SelectObject(&m_bmpBk);     //	CBitmap		m_bmpBk;
...
void CButtonST::DrawTransparent(BOOL bRepaint)
{
	...
	// Restore old bitmap (if any) 
	if (m_dcBk.m_hDC != NULL && m_pbmpOldBk != NULL)        //   CDC        m_dcBk;   HDC m_hDC; m_hDC是CDC类的一个成员变量；
	{ 
		m_dcBk.SelectObject(m_pbmpOldBk);                   //CBitmap*	m_pbmpOldBk;
	} // if
